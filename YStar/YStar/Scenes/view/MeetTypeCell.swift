...
class MeetTypeItemCell: UICollectionViewCell {
    ...
    func setMeetTypeItem(_ data: MeetTypeModel) {
        meetTypeImageView.kf.setImage(with: URL(string:qiniuHelper.shared().qiniuHeader +  data.showpic_url_tail), placeholder: UIImage.imageWith(AppConst.iconFontName.newsPlaceHolder.rawValue, fontSize: CGSize.init(width: 35, height: 35), fontColor: UIColor.init(rgbHex: AppConst.ColorKey.main.rawValue)))
