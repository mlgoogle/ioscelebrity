...
class MeetTypeDetailCell: OEZTableViewCell {
    ...
    override func update(_ data: Any!) {
        if let model = data as? MeetTypeModel{
            meetTypeImageView.kf.setImage(with: URL(string:qiniuHelper.shared().qiniuHeader +  model.showpic_url_tail), placeholder: UIImage.imageWith(AppConst.iconFontName.newsPlaceHolder.rawValue, fontSize: CGSize.init(width: 35, height: 35), fontColor: UIColor.init(rgbHex: AppConst.ColorKey.main.rawValue)))
